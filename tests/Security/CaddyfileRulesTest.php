...
/**
 * Розбирає dev/config/caddy/Caddyfile у впорядкований список правил і звіряє, що
 * кожне робить: директиву, її матчер і позицію в ланцюжку. Перевірка наявності
 * рядка тут не годиться — шлях у матчері лишається на місці й тоді, коли
 * заборона підмінена дозволом.
 *
 * У Caddy виграє перше правило, що збіглося, а не найточніше, тож позиція —
 * частина політики, а не оформлення.
 */
class CaddyfileRulesTest extends TestCase
{
    ...
    /**
     * Вебсервер і PHP тут один процес під одним UID, тож код застосунку дістає
     * до 127.0.0.1:2019, а POST /load замінює конфігурацію в памʼяті в обхід
     * білого списку.
     */
    ...
    /**
     * Фолбек вітрини мусить бути останнім правилом ланцюжка. Пару
     * `rewrite * /index.php` + `php` не можна шукати по всьому файлу: та сама
     * пара є у вкладеному route для files/originals/.
     */
    ...
    public function testDenialsComeBeforeTheAllowances(): void
    {
        ...
        // Винятки тримаються лише порядком: у Go RE2 немає негативного lookahead.
        $this->assertLessThan(
    ...
    /**
     * files/originals/ віддає сторінку 404 магазину, а не голу серверну. Шлях
     * шукається в розібраному матчері, а не в тексті рядка: інакше перевірка
     * реагує на позицію слова.
     */
