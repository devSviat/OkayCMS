...
Каталог `modules/` у темі — механізм перевизначення: якщо тема має власну версію шаблону,
стилю чи скрипта модуля, береться вона, а не файл із модуля. Це дозволяє правити вигляд
модуля, не редагуючи сам модуль.
