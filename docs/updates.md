...
Файли лежать у `release-migrations/` у корені репозиторію ядра — це вхід
збірки пакета (`ok release:build-package --migrations=…`, типове значення
саме цей каталог). У репозиторії його поки немає: жодна версія форку не
змінювала схему. Перша міграція створює каталог разом із собою.
