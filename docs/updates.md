...
до того ж не входить у `release-manifest.json`, тож у дерево інсталяції
цей каталог не копіюється взагалі.

До бази міграції доїжджають іншим шляхом — усередині пакета релізу, який
розпаковується в `files/tmp/updates/{версія}/extracted/migrations/`. Туди
веб-сервер не пускає: перевірено запитом, `.sql` і `.php` звідти віддають
404.
