...
- **Розташування.** Самооновлення живе в ядрі (`Okay/Core/Update/`,
  ...
  не треба.
- **Якщо оновлюєтесь із версії, де це був модуль** (`*/CoreUpdater` до
  v1.3.0): `apply` нічого не видаляє, тож каталог модуля лишиться на диску
  й далі реєструватиме свій пункт меню — тобто в адмінці буде **два**
  «Оновлення ядра». Приберіть і каталог, і рядок модуля: спершу видаліть
  модуль у «Мої модулі» (це прибере рядок з `ok_modules`), потім видаліть
  каталог. Порядок не критичний — осиротілий рядок без каталогу лише пише
  попередження в лог і пропускається.
