...
У поточному коді це роблять контролери кошика, списку бажань, порівняння, зворотного зв'язку,
підписки й швидкого замовлення, а через сервіс `Okay\Core\Security\StorefrontGuard` — ще й
хелпери, які самі обробляють POST: `CommonHelper` (зворотний дзвінок) і `CommentsHelper`.
Той самий сервіс беріть через DI там, де контролера немає.

Друга, окрема перевірка — **повтор** тієї самої відправки. CSRF-токен від нього не рятує:
подвійний клік несе цілком валідний токен. Метод, після якого зʼявляється рядок у базі або
йде лист, має спитати `Okay\Core\Security\FormToken::accept()` перед записом і на повтор
показати той самий успіх, а не помилку. Як це виглядає з боку форми й модуля —
[modules/frontend.md](modules/frontend.md#форма-в-модулі).

Деталі — [UPGRADE-security.md](UPGRADE-security.md), розмітка форм —
