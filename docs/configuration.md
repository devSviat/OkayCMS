...
| `db_timezone` | зсув часового поясу; типово закоментовано |
...
| Директива | Значення |
| --------- | -------- |
| `error_reporting`, `php_charset`, `php_locale_*` | застосовуються засобами PHP |
| `php_timezone` | передається в `date_default_timezone_set()` |
