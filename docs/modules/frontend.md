...
у запиті немає: тоді повтором вважається та сама відправка протягом заданого часу. Такий спосіб
менш точний, тому він запасний. Четвертим аргументом можна задати вікно:

| Вікно | Коли брати |
| ----- | ---------- |
| `FormToken::FINGERPRINT_TTL` (10 хв, типове) | дубль коштує дорого — замовлення |
| `FormToken::ACCIDENT_TTL` (1 хв) | дубль дешевий — заявка, відгук, коментар |

Коротке вікно там, де дубль дешевий, не випадкове: випадковий повтор стається за секунди, а
за десять хвилин покупець цілком може свідомо звернутись іще раз — і ця заявка має дійти.

Після успішного запису прив'яжіть результат до токена:

```php
FormToken::remember(self::ORDER_FORM, $this->request->post('form_token'), $order->url);
```

Тоді повтор саме з цієї сторінки отримає `FormToken::recall(...)` — рівно свій обʼєкт, а не
«останній створений у сесії», яким міг бути й зовсім інший.

Чотири речі, на яких легко спіткнутись:
...
- **Не беріть у `$payload` те, що змінює сама дія.** Оформлення очищає кошик, тож у другого
  ...
  руйнує. З тієї ж причини кладіть у `$payload` ціль дії: без неї однаковий короткий відгук на
  двох різних товарах дає той самий відбиток.
- **Токен — не єдиний рубіж.** Він видається на кожен рендер, тож дві вкладки з тією самою
  формою для нього однаково нові. Там, де це насправді дубль (оформлення: кошик один на
  сесію), потрібна ще перевірка стану — порожній кошик означає, що замовлення вже оформлене.
