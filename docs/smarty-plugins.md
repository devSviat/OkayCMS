...
токенів окремо для кожного `name=`, тож форми на одній сторінці незалежні.

`name=` обовʼязковий: без нього плагін друкує лише HTML-коментар і пише попередження в лог.
Мовчазний порожній вивід лишав би форму без захисту, і побачити це можна було б хіба по
дублях у базі.
