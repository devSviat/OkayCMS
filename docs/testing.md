...
142 тестових класи. Найбільші групи:
...
| `tests/Security/` | 45 | CSRF вітрини й адмінки, походження запиту, вхід і відновлення пароля, вибирач файлів, обхід шляхів, заголовки, куки й сесії, SVG, хендлери логу |
| `tests/TplMod/` | 7 | розбір і друк шаблонів, збіг анкерів `modifications` |
| `tests/Admin/` | 16 | контролери й помічники адмінки |
| `tests/Design/` | 6 | компіляція шаблонів тем і заборонені конструкції Smarty 5 |
| `tests/Modules/`, `tests/Helpers/`, `tests/Seo/`, `tests/Entities/`, `tests/Requests/` | 23 | решта |
...
| `Entities/SchemaMatchesEntitiesTest` | оголосити сутності поле, якого немає ні в `okay_clean.sql`, ні в core-міграціях форку — свіжий стенд народиться з відсталою схемою, а сторінка мовчки віддасть порожній список |

Причини кожного описані в докблоці самого тесту — там же історія, чому він з'явився.

### Набір перевірений мутацією

Кожен із **79 класів**, що існували на момент тієї перевірки, пройшов мутацію: у код чи артефакт вносилась саме та поломка, яку тест
...
Друга міна тієї ж родини — база. У CI-джобі PHPUnit її немає, а
`Okay\Core\QueryFactory\AbstractQuery` у конструкторі бере `Database` через `ServiceLocator` і
одразу конектиться. Тобто досить **створити** `$queryFactory->newSelect()`, навіть не виконуючи
запит: локально зелено, у CI `PDOException`. Вибірки для юніт-тесту будуйте голою
`Aura\SqlQuery\QueryFactory` — усі потрібні методи є в ній самій, а сутності всередині фільтрів
працюють із будь-яким `SelectInterface`. Перевіряти зручно на **паузі** контейнера бази: правильний
тест проходить за мілісекунди, а той, що тягне PDO, висить до таймауту, бо на паузі з'єднання не
відпадає з помилкою.
