...
131 тестовий клас. Найбільші групи:

| Каталог | Класів | Про що |
| ------- | ------ | ------ |
| `tests/Core/` | 43 | Design і Smarty-плагіни, TemplateConfig, консоль, QueryFactory, модулі, сутності |
| `tests/Security/` | 42 | CSRF вітрини й адмінки, вхід і відновлення пароля, вибирач файлів, обхід шляхів, заголовки, куки, SVG |
...
Розділ «Isolation between requests» перевіряє те, чого не видно з одного запиту: дві сесії
покупців ходять поперемінно, і валюта, кошик та ідентифікатор сесії не мають перетікати, а
запит менеджера не має робити наступного аноніма менеджером. У classic mode ці перевірки
тривіально зелені — сенс вони мають у worker mode, і саме тому ганяються в обох.

Кожна з них має власний контроль: якщо дві сесії не розрізняються з самого початку або
менеджер не увійшов, перевірка каже, що нічого не довела, замість показати зелене.
...
| `dev/bin/smoke.sh` | поведінка піднятого стека, окремою джобою на матриці `classic` / `worker` |
...
Smoke ганяється в обох режимах FrankenPHP. Розбіжність між classic і worker — головний ризик
worker mode, тож один прогін не заміняє другий; що саме перевіряється, описано в
[deployment-frankenphp.md](deployment-frankenphp.md#worker-mode).
