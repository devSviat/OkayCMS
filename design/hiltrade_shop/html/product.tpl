        ...
        <div class="block product-page__gallery f_col f_col-lg-7 f_col-xl-7">
            <div class="block--boxed block--border boxed--stretch d-md-flex justify-content-between">
                {if $product->images}
                    {* Main product image *}
                    <div
                        class="gallery_image product-page__image {if $product->images|count == 1} product-page__image--full {/if} f_row justify-content-center">
                                    ...
                                    <a href="{$image->filename|resize:1800:1800:w}" data-fancybox="we2" class="swiper-slide">
                                        <picture>
                                            {if $settings->support_webp}
                                                <source type="image/webp" srcset="{$image->filename|resize:700:800:w|webp}">
                                            {/if}
                                            <source srcset="{$image->filename|resize:700:800:w}">
                                            <img {if $image@first} itemprop="image" {/if}
                                                src="{$image->filename|resize:700:800:w}" alt="{$product->name|escape}"
                            ...
                            <div class="stickers stickers_product-page">
                                {if $product->featured}
                                    <span class="sticker sticker--hit"
                                        data-language="product_sticker_hit">{$lang->product_sticker_hit}</span>
                                {/if}
                                {if $product->variant->compare_price}
                                    <span class="sticker sticker--discount"
                                        data-language="product_sticker_discount">{$lang->product_sticker_discount}</span>
                                {/if}
                                {if $product->special}
                                    <span class="sticker sticker--special">
                                        <img class="sticker__image" src='files/special/{$product->special}'
                    ...
                    {if $product->images|count > 1}
                        <div
                            class="product-page__images swiper-container gallery-thumbs d-md-flex justify-content-center justify-content-md-start flex-md-column hidden-sm-down">
                            <div class="swiper-wrapper">
        ...
        <div class="block product-page__details f_col f_col-lg-5 f_col-xl-5">
            <div class="block--border boxed--stretch details_boxed">
                ...
                <div class="details_boxed__item">
                    <div class="details_boxed__header header_product">
                        <h1 class="header_product__title">
                            <span data-product="{$product->id}" itemprop="name">{$h1|escape}</span>
                        </h1>
                    </div>
                </div>
                ...
                <div class="details_boxed__item details_boxed__item--one">
                            ...
                            <div id="product_{$product->id}" class="product__rating fn_rating"
                                ...
                                {if $product->rating > 0}
                                    <span class="rating_text">( <span
                                            itemprop="reviewCount">{$product->votes|string_format:"%.0f"}</span> )</span>
                                    <span class="rating_text hidden">( <span
                                            itemprop="ratingValue">{$product->rating|string_format:"%.1f"}</span> )</span>
                            ...
                            {if !empty($brand->image)}
                                <div class="details_boxed__brand clearfix">
                                    <a href="{url_generator route="brand" url=$brand->url}">
                                        <img class="brand_img"
                                            src="{$brand->image|resize:120:65:false:$config->resized_brands_dir}"
                                            alt="{$brand->name|escape}" title="{$brand->name|escape}">
                            ...
                            {else}
                                <div class="details_boxed__no_brand clearfix">
                                    <span class="details_boxed__title"
                                        data-language="product_brand_name">{$lang->product_brand_name}</span>
                    ...
                    <div class="details_boxed__available">
                        <div class="available__no_stock d-flex align-items-center icon icon-highlight-off fn_not_stock{if $product->variant->stock > 0} hidden-xs-up{/if}"
                            data-language="product_out_of_stock">{$lang->product_out_of_stock}</div>
                        <div class="available__in_stock d-flex align-items-center icon icon-check-circle-outline fn_in_stock{if $product->variant->stock < 1} hidden-xs-up{/if}"
                            data-language="product_in_stock">{$lang->product_in_stock}</div>
                    </div>
                </div>
                ...
                <div class="details_boxed__item">
                    <form class="fn_variants" action="{url_generator route="cart"}">
                        ...
                        <div class="details_boxed__select">
                            <div class="details_boxed__title {if $product->variants|count < 2} hidden{/if}"
                                data-language="product_variant">{$lang->product_variant}:</div>
                            <select name="variant"
                                class="fn_variant variant_select {if $product->variants|count < 2} hidden {else}fn_select2{/if}">
                                {foreach $product->variants as $v}
                                    <option{if $product->variant->id == $v->id} selected{/if} value="{$v->id}"
                                        data-price="{$v->price|convert}" data-stock="{$v->stock}" {if $v->compare_price > 0}
                                            data-cprice="{$v->compare_price|convert}"
                                            {if $v->compare_price>$v->price && $v->price>0}
                                                data-discount="{round((($v->price-$v->compare_price)/$v->compare_price)*100, 2)}&nbsp;%"
                                                {/if}{/if}{if $v->sku} data-sku="{$v->sku|escape}" {/if}
                                ...
                                <div class="details_boxed__offer" itemprop="offers" itemscope=""
                                    ...
                                    <span class="hidden">
                                        <link itemprop="url"
                                            href="{url_generator route="product" url=$product->url absolute=1}" />
                                    ...
                                    <div class="d-flex flex-wrap align-items-center details_boxed__price_amount">
                                        <div class="d-flex align-items-center details_boxed__prices">
                                            {* Old price *}
                                            <div
                                                class="d-flex align-items-center details_boxed__old_price {if !$product->variant->compare_price} hidden-xs-up{/if}">
                                                <span class="fn_old_price">{$product->variant->compare_price|convert}</span>
                                                <span class="currency">{$currency->sign|escape}</span>
                                            </div>
                                            {* Price *}
                                            <div
                                                class="d-flex align-items-center details_boxed__price {if $product->variant->compare_price} price--red{/if}">
                                                <span class="fn_price" itemprop="price"
                                                    content="{$product->variant->price|convert:null:false}">{$product->variant->price|convert}</span>
                                    ...
                                    <div class="d-flex align-items-center details_boxed__buttons">
                                        ...
                                        {else}
                                            ...
                                            <div class="fn_is_preorder {if $product->variant->stock > 0} hidden-xs-up{/if}">
                                                <button class="product-page__button product-page__button--preloader" type="submit"
                                        ...
                                        <div class="d-flex align-items-center details_boxed__other">
                                            ...
                                            {if is_array($wishlist->ids) && in_array($product->id, $wishlist->ids)}
                                                <a href="#" data-id="{$product->id}"
                                                    class="fn_wishlist product-page__wishlist selected"
                                            ...
                                            {else}
                                                <a href="#" data-id="{$product->id}" class="fn_wishlist product-page__wishlist"
                                            ...
                                            {else}
                                                <a class="fn_comparison product-page__compare" href="#" data-id="{$product->id}"
                                    ...
                                    {foreach $product->features as $f}
                                        <li class="f_col-md-6 features__item">
                                            <div class="d-flex justify-content-start features__wrap">
                                                <div class="features__name"><span>{$f->name|escape}:</span>{if $f->description}<span
                                                            title="{$f->description}"
                                                        style="margin-left: 10px; cursor: pointer; background: lightgreen; padding: 5px; border-radius: 5px;">i</span>{/if}
                                                </div>
                                                <div class="features__value">
                                                    {foreach $f->values as $value}
                                                        {if $category && $f->url_in_product && $f->in_filter && $value->to_index}
                                                            <a
                                                                href="{url_generator route="category" url=$category->url}{if !$settings->category_routes_template_slash_end}/{/if}{$f->url}-{$value->translit}">{$value->value|escape}</a>{if !$value@last},{/if}{*todo генерация урла*}
                                                        {else}
                                                ...
                                                <div class="comment__item {if $level > 0} admin_note{/if}">
                                                    ...
                                                    <div class="comment__inner">
                                                        ...
                                                        <div class="comment__boxed">
                                                            <div
                                                                class="d-flex flex-wrap align-items-center justify-content-between comment__header">
                                                                {* Comment name *}
                                    ...
                                    <form id="captcha_id" class="form form--boxed fn_validate_product" method="post">
                                        ...
                                        <div class="form__header">
                                            <div class="form__title">
                                                {include file="svg.tpl" svgId="comment_icon"}
                                                <span
                                                    data-language="product_write_comment">{$lang->product_write_comment}</span>
                                            </div>
                                        ...
                                        <div class="form__body">
                                            ...
                                            <div class="form__group">
                                                <textarea class="form__textarea form__placeholder--focus" rows="3"
                                                    name="text">{$request_data.text}</textarea>
                                        ...
                                        <div class="form__footer">
                                            ...
                                            {if $settings->captcha_comment}
                                                ...
                                                {elseif $settings->captcha_type == "default"}
                                                    ...
                                                    <div class="captcha">
                                                        <div class="secret_number">{$captcha_comment[0]|escape} + ? =
                                                            {$captcha_comment[1]|escape}</div>
