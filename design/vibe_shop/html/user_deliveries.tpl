                        ...
                        {if $delivery->image}
                            <span class="vs-option-card__logo">
                                <picture>
                                    ...
                                    <img class="lazy" data-src="{$delivery->image|resize:80:30:false:$config->resized_deliveries_dir}" src="{$rootUrl}/design/{get_theme}/images/xloading.svg" alt="{$delivery->name|escape}"/>
                                </picture>
                            </span>
                        {/if}
                    </label>
                ...
                <div class="vs-options">
                    {foreach $payment_methods as $payment_method}
                        <div class="payment_method__item fn_payment_method__item fn_payment_method__item_{$payment_method->id} vs-option"{if !in_array($payment_method->id, $active_delivery->payment_methods_ids)} style="display: none;" {/if}>
                            <label class="checkbox delivery__label vs-option-card{if $active_payment->id==$payment_method->id} active{/if}" for="payment_{$payment_method->id}">
                                <input class="delivery__input vs-option-card__radio" id="payment_{$payment_method->id}" type="radio" name="payment_method_id" data-currency_id="{$payment_method->currency_id}" value="{$payment_method->id}"{if $active_payment->id==$payment_method->id} checked{/if} />
                                <span class="vs-option-card__body">
                                ...
                                {if $payment_method->image}
                                    <span class="vs-option-card__logo">
                                        <picture>
                                            ...
                                            <img class="lazy" data-src="{$payment_method->image|resize:80:30:false:$config->resized_payments_dir}" src="{$rootUrl}/design/{get_theme}/images/xloading.svg" alt="{$payment_method->name|escape}"/>
                                        </picture>
                                    </span>
                                {/if}
                            </label>
