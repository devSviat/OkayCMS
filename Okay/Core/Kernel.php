...
final class Kernel
{
    ...
    {
        // Межа запиту оголошується явно, а не лишається на розсуд SAPI: під
        // FrankenPHP сесію однаково закриє завершення запиту, але доти файл
        // сесії тримає лок, і паралельні запити того самого покупця стають у
        // чергу. startFrontend() до того ж виходить одразу на активній сесії.
        //
        // Скидати ідентифікатор тут не можна: session_id('') означає для PHP
        // "створити нову", а не "прочитати куку", тож кожен покупець діставав
        // би порожню сесію. Куку читає сам session_start().
