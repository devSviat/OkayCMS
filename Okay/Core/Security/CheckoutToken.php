 ...
 * повтору того самого: подвійний клік, F5 на сторінці результату, повтор
 * після таймауту. Кожен такий повтор створював друге замовлення й другий
 * лист покупцю та адміністратору.
 */
class CheckoutToken
{
    ...
    /**
     * Успішна перевірка одразу знімає токен: другий запит із тим самим
     * значенням уже не пройде.
     */
    ...
    /**
     * Запасний шлях для тем, які ще не передають checkout_token: той самий
     * склад замовлення протягом TTL вважається повтором.
     *
     * Точність нижча за токен - навмисне повторне замовлення того самого
     * набору за 10 хвилин теж відсіється - тому це саме запасний шлях,
     * а не основний.
     */
