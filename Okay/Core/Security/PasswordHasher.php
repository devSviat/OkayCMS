...
/**
 * Єдина точка перевірки та створення хешів паролів.
 *
 * Нові паролі завжди створюються через password_hash(). Старі формати
 * (APR1-MD5, salted MD5, raw MD5) перевіряються лише заради зворотної
 * сумісності й мають бути перехешовані після успішного входу.
 */
class PasswordHasher
{
    /** Хеш у форматі APR1-MD5: $apr1$<salt 1-8>$<22 символи> */
    const APR1_PATTERN = '/^\$apr1\$([.\/0-9A-Za-z]{1,8})\$[.\/0-9A-Za-z]{22}$/';

    /** Будь-який MD5-хеш (salted або raw) */
    ...
    /**
     * Залишено для перевірки наявних APR1-хешів.
     * Нові паролі цим методом не створюються.
     */
